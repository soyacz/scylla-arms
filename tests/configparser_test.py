...
def test_read_properties():
    assert os.path.exists('tests/sample_file/general.properties')
    general_p = properties_parser('tests/sample_file/general.properties')
    build_metadata_file = general_p.get('buildMetadataFile')
    assert build_metadata_file == '00-Build.txt'
    assert os.path.exists('tests/sample_file/branch-specific.properties')
    branch_p = properties_parser('tests/sample_file/branch-specific.properties')
    product_name = branch_p.get('productName')
    assert product_name == 'scylla'

def test_read_build_metadata():
    assert os.path.exists('tests/sample_file/00-Build.txt')
    metadata_p = build_metadata_parser('tests/sample_file/00-Build.txt')
